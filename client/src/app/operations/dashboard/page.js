...
export default function OperationsDashboard() {
  ...
  const { stats, loading, error } = useSelector((state) => state.operationsStats);
  ...
  if (loading)
    return (
      <div className="flex justify-center items-center h-screen bg-gray-50">
        <p className="text-lg font-semibold text-gray-700">Loading operations stats...</p>
      </div>
    );
...
const renderLabel = ({
  ...
  // Normal label position for non-zero values
  let radius = outerRadius + 25;
  let x = cx + radius * Math.cos(-midAngle * RADIAN);
  let y = cy + radius * Math.sin(-midAngle * RADIAN);

  //  FIX: Move zero-value labels away to avoid overlap
  if (value === 0) {
    x = cx + outerRadius + 20; // push right side
    y = cy + index * 15;       // stack vertically using index
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <h1 className="text-3xl font-bold mb-8 text-gray-800">Operations Dashboard 🛠️</h1>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-md">
        <h3 className="text-xl font-semibold mb-4 text-gray-700">Order Breakdown</h3>
