...
export default function DocumentUpload() {
  ...
  const { loadingUpload, documents: uploadedDocs } = useSelector(state => state.orderDocuments);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!Object.keys(uploadedFiles).length) return alert("Select files to upload");
  ...
  return (
    <div className="container mx-auto py-8 max-w-3xl">
      <h2 className="text-2xl font-bold mb-6">{serviceName} - Upload Documents</h2>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-6">
        {documents.map(doc => (
          <div key={doc.id} className="flex flex-col">
            <label className="font-semibold mb-1">{doc.doc_name}{doc.is_mandatory ? "*" : ""}</label>
            <input
              type="file"
            ...
            {/* Show already uploaded files for this document */}
            {uploadedDocs?.filter(f => f.service_doc_id === doc.id)?.length > 0 && (
